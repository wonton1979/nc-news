import {getArticlesByQuery, getArticlesByQueryByPage} from "../../fetchData.js";
...
export default function ArticlesBySort () {
    ...
    useEffect(() => {
        getArticlesByQuery(searchParams.get("topic")).then(({data:{articles}}) => {
            setPages(Math.ceil(articles.length/8));
            getArticlesByQueryByPage(searchParams.get("topic"),1).then(({data:{articles}}) => {
    ...
    if (loading) {
        ...
        function fetchArticlesByPage(event){
            ...
            getArticlesByQueryByPage(searchParams.get("topic"),pageClicked).then(({data:{articles}}) => {
                setArticleList(articles);
