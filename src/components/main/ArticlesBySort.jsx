import {getArticlesByQueryTopic, getArticlesByQueryByPageTopic} from "../../fetchData.js";
...
export default function ArticlesBySort () {
    ...
    function handleFilterSearch(){
        setFilterSearchPath( `?topic=${searchParams.get("topic")}&sort_by=${filterSelected}`+ "&order="+orderBy)
    }

    useEffect(() => {
        if(filterSearchPath===""){
            getArticlesByQueryTopic(`?topic=${searchParams.get("topic")}`).then(({data:{articles}}) => {
                setPages(Math.ceil(articles.length/8));
                getArticlesByQueryByPageTopic(`?topic=${searchParams.get("topic")}`,1).then(({data:{articles}}) => {
            ...
            getArticlesByQueryTopic(filterSearchPath).then(({data:{articles}}) => {
                setPages(Math.ceil(articles.length/8));
                getArticlesByQueryByPageTopic(filterSearchPath,1).then(({data:{articles}}) => {
                    setArticleList(articles);
                    if(articles.length === 0){
                        setIsEmptyContent(true);
                    }
    ...
    if (loading) {
        ...
        function fetchArticlesByPage(event){
            ...
            if(filterSearchPath===""){
                getArticlesByQueryByPageTopic(`?topic=${searchParams.get("topic")}`,pageClicked).then(({data:{articles}}) => {
                    setArticleList(articles);
                    setCurrentPage(pageClicked);
                });
            }
            else {
                getArticlesByQueryByPageTopic(filterSearchPath,pageClicked).then(({data:{articles}}) => {
                    setArticleList(articles);
                    setCurrentPage(pageClicked);
                });
            }
                ...
                <>
                    <div className="relative flex justify-end items-center text-left mr-7">
                        ...
                        focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-8 py-2 ml-5 text-center
                         dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 hidden sm:block" onClick={handleFilterSearch}>
                                Search Articles
                            </button>

                            <button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4
                        focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-8 py-2 ml-5 text-center
                         dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 hidden sm:block" onClick={()=>{window.location.reload()}}>
                                Reset Filters
                            </button>
                        </div>
                    </div>
